...
// 		setSelectedOptionId(id);
// 		setSelectedOptionName(name);
// 		// Logic to open form by ID, if needed
// 	};
...
// 											}
// 										/>
// 										<div className="mt-2">{handleAttributeValueComponent(i)}</div>
// 									</div>
...
// 													handleOptionClick(combinedId, e.combinedName);
// 												} else {
// 													handleAppend(); // Append new item when option is clicked
// 												}
// 											}}
// 										>
// 											<p className="fill-title">{e.combinedName || e.name}</p>
...
export default function AddOptionManager<TFormStore>({ formStore, label }: Props<TFormStore>) {
	...
	const [selectedOptionId, setSelectedOptionId] = useState<string | null>(null);
	const [selectedOptionName, setSelectedOptionName] = useState<string | null>(null);
	const [newItemIndex, setNewItemIndex] = useState<number | null>(null); // State to track new item index
	const { t } = useTranslation();
	const { attributes } = useAppSelector((state) => state.attributes);
	...
	const handleAddOptions = () => {
		...
		const updatedOptions = [...selectedOptions, ...newOptions];
		...
		if (formStore.control) {
			...
			formStore.control.setValue('variants', updatedFields);
		}
	...
	const handleAppend = () => {
		const index = fields.length; // Get the new index
		append({
			sku: '',
			en: {
				name: '',
		...
		setNewItemIndex(index); // Set the index of the newly appended item
	};
			...
			{fields?.length > 0 &&
				fields.map((item, i) => (
					<div key={i}>
						...
						{/* Render FormOptionById component if there's a new item */}
						{newItemIndex !== null && (
							<FormOptionById
								formStore={formStore}
								index={newItemIndex}
